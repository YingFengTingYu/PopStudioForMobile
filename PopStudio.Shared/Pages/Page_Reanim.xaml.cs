...
namespace PopStudio.Pages
{
    ...
    public sealed partial class Page_Reanim : Page, IMenuChoosable
    {
        ...
        public static string StaticTitle { get; set; } = YFString.GetString("Reanim_Title");
        ...
        private async void ButtonRun_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                        ...
                        foreach (YFFileSystem.YFFile f in m_dir_in.GetAllFiles())
                        {
                            if (f.Name.ToLower().EndsWith(inFormat) && !m_dir_out.DirectoryExist(f.Name))
                            {
                                ...
                                taskList.Add(Task.Run(() =>
                                {
                                    try
                                    {
                                        YFAPI.TranscodeReanim(f, o, inmode, outmode);
                                    }
                                    catch (Exception)
                                    {
                                    }
                ...
                else
                {
                    ...
                    taskList.Add(Task.Run(() => YFAPI.TranscodeReanim(inFile, outFile, inmode, outmode)));
                }
