...
{
    /// <summary>
    /// 这玩意不太方便，不提供解码块的函数，只提供解码整个图的函数
    /// </summary>
    internal static unsafe class PVRTC
    {
        ...
        /// <summary>
        /// PowerVR SDK提供RGBA_PVRTCI_4/2BPP解码
        /// 我魔改了一下，让他支持RGB_PVRTCI_4/2BPP和RGBA_PVRTCII_4/2BPP解码
        /// 添加了PVRTCII支持，PVRTC HDR支持或许随后会添加
        /// 源文件地址：
        ...
        private static class PVRTCDecoder
        {
            ...
            /// <summary>
            /// 在颜色A/B和调制模式数据中解码出颜色A（RGBA5554）
            /// </summary>
            /// <param name="ColorData">颜色A/B和调制模式数据</param>
            /// <param name="versionII">是否为PVRTCII纹理</param>
            /// <returns>颜色A，RGBA5554</returns>
            static YFColor GetColorA(uint ColorData, bool versionII)
            {
                YFColor color;
            ...
            /// <summary>
            /// 在颜色A/B和调制模式数据中解码出颜色B（RGBA5554）
            /// </summary>
            /// <param name="ColorData">颜色A/B和调制模式数据</param>
            /// <param name="versionII">是否为PVRTCII纹理</param>
            /// <returns>颜色B，RGBA5554</returns>
            static YFColor GetColorB(uint ColorData, bool versionII)
            {
                YFColor color;
                ...
                {
                    uint adds = versionII ? 1U : 0U; // 在PVRTCII中，为了使Alpha横跨整个色域，使用A(3)A(2)A(1)1格式
            ...
            /// <summary>
            /// 将RGBA5554颜色解码为RGBA8888颜色
            /// </summary>
            /// <param name="color5554">RGBA5554颜色</param>
            /// <returns>RGBA8888颜色</returns>
            static Pixel128S Color5554To8888(YFColor color5554)
            {
            ...
            /// 从4个颜色插值为4*4或8*4的块
            /// </summary>
            /// <param name="P">左上角P点颜色</param>
            /// <param name="Q">右上角Q点颜色</param>
            /// <param name="R">左下角R点颜色</param>
            /// <param name="S">右下角S点颜色</param>
            /// <param name="pPixel">插值颜色指针</param>
            /// <param name="bpp">平均每个点占据位数</param>
            ...
            /// <param name="offsetX">相对x坐标，确定这个四分之一块的相对位置</param>
            /// <param name="offsetY">相对y坐标，确定这个四分之一块的相对位置</param>
            /// <param name="ModulationValues">索引指针</param>
            /// <param name="ModulationModes">调制模式指针</param>
            /// <param name="bpp">平均每个点占据位数</param>
            ...
            static Pixel128S GetPaletteColor(Pixel128S Pa, Pixel128S Pb, Pixel128S Qa, Pixel128S Qb, Pixel128S Ra, Pixel128S Rb, Pixel128S Sa, Pixel128S Sb, uint x, uint y, int modvalue)
            {
                if (y == 0)
                {
                    if (x == 0)
            ...
            /// <summary>
            /// 获取PVRTCWord的真实索引（因为输入的索引有可能大于等于Word数量）
            /// </summary>
            /// <param name="numWords">Word数量</param>
            /// <param name="word">Word索引</param>
            /// <returns>PVRTCWord的真实索引</returns>
            ...
            /// <summary>
            /// 通过大小和坐标获取序号，PVRTCI按照反莫顿码方式存储，PVRTCII直接按顺序存储
            /// </summary>
            /// <param name="XSize">宽度，传入图像宽度除以块宽度即可</param>
            /// <param name="YSize">高度，传入图像高度除以块高度即可</param>
            /// <param name="XPos">PVRTCWord的x坐标</param>
            /// <param name="YPos">PVRTCWord的y坐标</param>
            /// <param name="versionII">是否为PVRTCII纹理</param>
            /// <returns>序号</returns>
            static uint GetIndex(uint XSize, uint YSize, uint XPos, uint YPos, bool versionII)
            {
                if (versionII)
                {
            ...
            /// 将解码所得数据复制到图像数据
            /// </summary>
            /// <param name="pOutput">解码图像指针</param>
            /// <param name="width">图像宽度</param>
            /// <param name="pWord">解码数据指针</param>
            /// <param name="words">包含四个PVRTCWord坐标的东西</param>
            /// <param name="bpp">平均每个点占据位数</param>
            /// <param name="hasA">是否解码Alpha通道</param>
            static void MapDecompressedData(YFColor* pOutput, int width, YFColor* pWord, PVRTCWordIndices words, byte bpp, bool hasA)
            {
                uint WordWidth = bpp == 2 ? 8U : 4U;
                uint WordHeight = 4;
                uint dw = WordWidth >> 1;
            ...
            /// 解码PVRTC数据
            /// </summary>
            /// <param name="pCompressedData">PVRTC数据指针</param>
            /// <param name="pDecompressedData">图像指针</param>
            /// <param name="Width">图像宽度</param>
            ...
            {
                if (!(bpp == 2 || bpp == 4 || bpp == 6 || bpp == 8))
                {
                    return;
                }
                if (bpp > 4)
                {
                    DecompressPVRTCHDR(pCompressedData, pDecompressedData, Width, Height, (byte)(bpp - 4), versionII, hasA);
                }
                else
                {
                    DecompressPVRTCLDR(pCompressedData, pDecompressedData, Width, Height, bpp, versionII, hasA);
                }
            }

            static void DecompressPVRTCHDR(byte* pCompressedData, YFColor* pDecompressedData, uint Width, uint Height, byte bpp, bool versionII, bool hasA)
            {
                uint WordWidth = bpp == 2 ? 8U : 4U;
                uint WordHeight = 4;
                ...
                int WordSquare = (int)(WordWidth * WordHeight);
                YFColor* pPixels = stackalloc YFColor[WordSquare << 1];
                YFColor* pPixels_out = stackalloc YFColor[WordSquare];
                ...
                // 在栈上分配二维数组
                int* int_buffer = stackalloc int[16 * 8 + 16 * 8];
                int** ModulationValues = stackalloc int*[16]; // [16][8]
                for (int i = 0; i < 16; i++)
                {
                    ModulationValues[i] = int_buffer;
                    int_buffer += 8;
                }
                int** ModulationModes = stackalloc int*[16]; // [16][8]
                for (int i = 0; i < 16; i++)
                {
                ...
                for (int wordY = -1; wordY < NumYWords - 1; wordY++)
                {
                    for (int wordX = -1; wordX < NumXWords - 1; wordX++)
                    {
                        ...
                        WordOffsets[0] = GetIndex((uint)NumXWords, (uint)NumYWords, (uint)indices.P[0], (uint)indices.P[1], versionII) << 2;
                        WordOffsets[1] = GetIndex((uint)NumXWords, (uint)NumYWords, (uint)indices.Q[0], (uint)indices.Q[1], versionII) << 2;
                        WordOffsets[2] = GetIndex((uint)NumXWords, (uint)NumYWords, (uint)indices.R[0], (uint)indices.R[1], versionII) << 2;
                        WordOffsets[3] = GetIndex((uint)NumXWords, (uint)NumYWords, (uint)indices.S[0], (uint)indices.S[1], versionII) << 2;
                        PVRTCWord P, Q, R, S;
                        // 第一组信息是指数？信息
                        ...
                        // 第二组信息是颜色信息
                        P.ColorData = pWordMembers[WordOffsets[0] + 3];
                        P.ModulationData = pWordMembers[WordOffsets[0] + 2];
                        Q.ColorData = pWordMembers[WordOffsets[1] + 3];
                        Q.ModulationData = pWordMembers[WordOffsets[1] + 2];
                        R.ColorData = pWordMembers[WordOffsets[2] + 3];
                        R.ModulationData = pWordMembers[WordOffsets[2] + 2];
                        S.ColorData = pWordMembers[WordOffsets[3] + 3];
                        S.ModulationData = pWordMembers[WordOffsets[3] + 2];
                        PvrtcGetDecompressedPixels(P, Q, R, S, pPixels + WordSquare, bpp, ModulationValues, ModulationModes, upscaledColorA, upscaledColorB, versionII);
                        // 将两组信息解码
                        DecodeHDRColor(pPixels, pPixels + WordSquare, pPixels_out, bpp);
                        MapDecompressedData(pDecompressedData, (int)Width, pPixels_out, indices, bpp, hasA);
                    }
                }
            }

            static void DecodeHDRColor(YFColor* group1, YFColor* group2, YFColor* outcolor, byte bpp)
            {
                int WordSquare = (bpp == 2 ? 8 : 4) * 4;
                for (int i = 0; i < WordSquare; i++)
                {
                    //*outcolor++ = *group2++;
                    outcolor->Red = ToByte((group1->Red + group2->Red) >> 1);
                    outcolor->Green = ToByte((group1->Green + group2->Green) >> 1);
                    outcolor->Blue = ToByte((group1->Blue + group2->Blue) >> 1);
                    outcolor->Alpha = ToByte((group1->Alpha + group2->Alpha) >> 1);
                    group1++;
                    group2++;
                    outcolor++;
                }
            ...
            static void DecompressPVRTCLDR(byte* pCompressedData, YFColor* pDecompressedData, uint Width, uint Height, byte bpp, bool versionII, bool hasA)
            {
                uint WordWidth = bpp == 2 ? 8U : 4U;
                uint WordHeight = 4;
                uint* pWordMembers = (uint*)pCompressedData;
        ...
        /// <summary>
        /// 谷歌image_compression提供RGBA_PVRTCI_2BPP编码
        /// 我魔改了一下，让他支持RGB_PVRTCI_4/2BPP，RGBA_PVRTCI_4BPP和RGBA_PVRTCII_4/2BPP编码
        /// 添加了PVRTCII支持（其实不会用硬过滤标志），PVRTC HDR支持或许随后会添加
        /// 源文件地址：
        ...
        private static class PVRTCEncoder
        {
            ...
            static YFColor Interpolate4_4BPP(YFColor color00, YFColor color01, YFColor color10, YFColor color11, uint px, uint py)
            {
                ...
                return new YFColor(
                    (a * color00.Red + b * color01.Red + c * color10.Red + d * color11.Red) /
                        downscale,
                    (a * color00.Green + b * color01.Green + c * color10.Green + d * color11.Green) /
                        downscale,
                    (a * color00.Blue + b * color01.Blue + c * color10.Blue + d * color11.Blue) /
                        downscale,
                    (a * color00.Alpha + b * color01.Alpha + c * color10.Alpha + d * color11.Alpha) /
                        downscale);
            }
