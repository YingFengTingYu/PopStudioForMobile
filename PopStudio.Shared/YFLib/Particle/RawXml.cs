...
namespace PopStudio.Particle
{
    internal class RawXml
    {
        public static Particle Decode(YFFile inFile)
        {
            ...
            particles.Emitters = new ParticleEmitter[EmitterNumber];
            List<ParticleField> EmitterField = new List<ParticleField>();
            List<ParticleField> EmitterSystemField = new List<ParticleField>();
            for (int i = 0; i < EmitterNumber; i++)
            {
                EmitterField.Clear();
                EmitterSystemField.Clear();
                XmlNode node_out = childlist[i];
                if (node_out.Name != "Emitter") throw new TypeMismatchException();
        ...
        static ParticleField ReadField(XmlNode root)
        {
        ...
        static ParticleTrackNode[] ReadTrackNode(string inText)
        {
            List<ParticleTrackNode> ans = new List<ParticleTrackNode>();
            ...
            ParticleTrackNode[] realans = ans.ToArray();
            //Default Times
        ...
        public static void Encode(Particle particles, YFFile outFile)
        {
            using (Stream stream = outFile.CreateAsStream())
            {
                ...
                {
                    ParticleEmitter[] emitters = particles.Emitters;
        ...
        static void WriteFields(ParticleField[] fields, StreamWriter sw, string FieldsName)
        {
            int length = fields.Length;
            if (length == 0) return;
            for (int i = 0; i < length; i++)
            {
                ParticleField field = fields[i];
                sw.Write("  <");
                sw.Write(FieldsName);
                sw.Write(">\n");
        ...
        static void WriteTrackNode(ParticleTrackNode[] track, StreamWriter sw)
        {
            int length = track.Length;
            for (int i = 0; i < length; i++)
            {
                if (i > 0) sw.Write(' ');
                ParticleTrackNode node = track[i];
