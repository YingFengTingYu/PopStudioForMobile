...
namespace PopStudio.Trail
{
    internal class WP
    {
        private static readonly byte[] _xnbMagic = new byte[0x6] { 0x58, 0x4E, 0x42, 0x6D, 0x05, 0x00 };
        private static readonly byte[] _xnbInfo = new byte[0x25] { 0x01, 0x1D, 0x53, 0x65, 0x78, 0x79, 0x2E, 0x54, 0x6F, 0x64, 0x4C, 0x69, 0x62, 0x2E, 0x54, 0x72, 0x61, 0x69, 0x6C, 0x52, 0x65, 0x61, 0x64, 0x65, 0x72, 0x2C, 0x20, 0x4C, 0x41, 0x57, 0x4E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 };

        public static void Encode(Trail trail, YFFile outFile)
        {
            using (BinaryStream bs = outFile.CreateAsBinaryStream())
            {
                bs.WriteBytes(_xnbMagic);
                long off_size = bs.Position;
                bs.Position += 4;
                bs.WriteBytes(_xnbInfo);
