...
{
    internal static unsafe class ETC
    {
        public static void EncodeBlock_RGB_ETC1(byte* texPtr, YFColor* color)
        {
            ulong ans = ETC1Coder.GenETC1(color);
            *texPtr++ = (byte)(ans >> 56);
            *texPtr++ = (byte)(ans >> 48);
            *texPtr++ = (byte)(ans >> 40);
            *texPtr++ = (byte)(ans >> 32);
        ...
        {
            ulong word = 0;
            word |= (ulong)*texPtr++ << 56;
            word |= (ulong)*texPtr++ << 48;
            word |= (ulong)*texPtr++ << 40;
            word |= (ulong)*texPtr++ << 32;
            word |= (ulong)*texPtr++ << 24;
            word |= (ulong)*texPtr++ << 16;
            word |= (ulong)*texPtr++ << 8;
            word |= *texPtr++;
            ETC1Coder.DecodeETC1(word, color);
        }

        public static class ETC1Coder
        {
