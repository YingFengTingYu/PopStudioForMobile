...
{
    public unsafe class RGB_ATC : ICoder
    {
        ...
        public void Decode(YFTexture2D tex, YFColor* dataPtr)
        {
            fixed (byte* tempPtr = tex.TexData)
            {
                ...
                for (int y = 0; y < height; y += 4)
                {
                    ...
                    {
                        CompressedCoder.ATC.DecodeBlock_RGB_ATC(texPtr, color_buffer);
                        texPtr += 8;
                        for (int i = 0; i < 4; i++)
                        {
                            for (int j = 0; j < 4; j++)
            ...
            fixed (byte* tempPtr = tex.TexData)
            {
                ...
                for (int y = 0; y < height; y += 4)
                {
                    for (int x = 0; x < width; x += 4)
                    {
                        ...
                        CompressedCoder.ATC.EncodeBlock_RGB_ATC(texPtr, color_buffer);
                        texPtr += 8;
                    }
                }
            }
