...
namespace PopStudio.Package.Rsb
{
    ...
    {
        private static string autopoolname = "_AutoPool";

        public static void Pack(YFDirectory inFolder, YFFile outFile)
        {
            string adds = inFolder.Name + "/";
            byte[] RsbPackBuffer = new byte[81920];
            YFDirectory studioPath = inFolder.GetYFDirectory("POPSTUDIOINFO");
            ...
            YFFile xmlPackInfoFile = studioPath.GetYFFile("PACKINFO.XML");
            if (xmlPackInfoFile is not null)
            {
                using (Stream stream = xmlPackInfoFile.OpenAsStream())
                {
            ...
            using BinaryStream bs_rsgpfile = new BinaryStream(new FileStream(tempFilePool.Add(), FileMode.Create));
            ...
            if (usePoolXml)
            {
                ...
                using (Stream stream = studioPath.GetYFFile("POOL.XML").OpenAsStream())
                {
                    using (StreamReader sr = new StreamReader(stream))
                    {
                        xmldata = sr.ReadToEnd();
                ...
                using (Stream stream = studioPath.GetYFFile("RESOURCESGROUP.XML").OpenAsStream())
                {
                    using (StreamReader sr = new StreamReader(stream))
                    {
                        xmldata = sr.ReadToEnd();
                ...
                for (int i = 0; i < childlist.Count; i++)
                {
                    ...
                    using (BinaryStream bsP0over = new BinaryStream())
                    {
                        using (BinaryStream bsP1over = new BinaryStream())
                        {
                                ...
                                using (BinaryStream bsP1 = new BinaryStream())
                                {
                                    ...
                                    for (int j = 0; j < childchildlist.Count; j++)
                                    {
                                        ...
                                        if (childchildlist[j].Name == "Img")
                                        {
                                            ...
                                            YFFile name = GetYFFileFromPath(adds + fileid, inFolder);
                                            bool delete = false;
                                            if (name is null)
                                            {
                                                int index = fileid.LastIndexOf('.');
                                                string changeId = index >= 0
                                                    ? fileid[..index]
                                                    : fileid;
                                                YFFile name2 = GetYFFileFromPath(adds + changeId + ".PNG", inFolder);
                                                if (name2 is not null)
                                                {
                                                    name = CreateYFFileFromPath(adds + fileid, inFolder);
                                                    int format = Convert.ToInt32(childchildlist[j].Attributes["defaultformat"].Value);
                                                    YFAPI.EncodeImage(name2, name, 0, format, bs_rsgpfile.Endian);
                                                    delete = true;
                                                }
                                            }
                                            using (BinaryStream bsindexunknow = name.OpenAsBinaryStream())
                                            {
                                                ...
                                                bsindexunknow.StaticCopyTo(bsP1, RsbPackBuffer);
                                                bsP1.Length = FourK(bsP1.Position);
                                                bsP1.Position = bsP1.Length;
                                            }
                                            ptxList.Add(ptx);
                                            if (delete) name?.DeleteSelf();
                                        }
                                        else
                                        {
                                            //p0
                                            using (BinaryStream bsindexunknow = GetYFFileFromPath(adds + childchildlist[j].Attributes["id"].Value, inFolder).OpenAsBinaryStream())
                                            {
                                                rsgp.fileList.Add(new CompressString(fileid, new RsgpPart0ExtraInfo((uint)bsP0.Position, (uint)bsindexunknow.Length)));
                                                bsindexunknow.StaticCopyTo(bsP0, RsbPackBuffer);
                                    ...
                                    if (compressPart0)
                                    {
                                        ...
                                        {
                                            bsP0.StaticCopyTo(zLibStream, RsbPackBuffer);
                                        }
                                    ...
                                    {
                                        bsP0.StaticCopyTo(bsP0over, RsbPackBuffer);
                                    }
                                    if (compressPart1)
                                    {
                                        using (ZLibStream zLibStream = new ZLibStream(bsP1over, level, true))
                                        {
                                            bsP1.StaticCopyTo(zLibStream, RsbPackBuffer);
                                        }
                                    }
                                    else
                                    {
                                        bsP1.StaticCopyTo(bsP1over, RsbPackBuffer);
                                    }
                            ...
                            bsP0over.StaticCopyTo(bs_rsgpfile, RsbPackBuffer);
                            ...
                            bsP1over.StaticCopyTo(bs_rsgpfile, RsbPackBuffer);
                        }
                    }
                    //write head
                ...
                : outFile.CreateAsBinaryStream())
            {
                ...
                using (Stream stream = studioPath.GetYFFile("COMPOSITERESOURCES.XML").OpenAsStream())
                {
                    using (StreamReader sr = new StreamReader(stream))
                    {
                        xmldata = sr.ReadToEnd();
                ...
                YFFile xmlpath = studioPath.GetYFFile("RESOURCES.XML");
                if (xmlpath is not null)
                {
                    ...
                    //    bs.Position = bs.Length;
                    //}
                    bs.Length = FourK(bs.Position); // For Console version
                    bs.Position = bs.Length;
                    using (BinaryStream bs_xml = new BinaryStream())
                    {
                        ...
                        bs_xml.StaticCopyTo(bs, RsbPackBuffer);
                    }
                }
                bs.Length = FourK(bs.Position);
                bs.Position = bs.Length;
                ...
                bs_rsgpfile.StaticCopyTo(bs, RsbPackBuffer);
                ...
                if (smf)
                {
                    using (BinaryStream bs_e = outFile.CreateAsBinaryStream())
                    {
                        bs.Position = 0;
                        ...
                        {
                            bs.StaticCopyTo(zLibStream, RsbPackBuffer);
                        }
                    }
                }
            }
        ...
        public static void Unpack(YFFile inFile, YFDirectory outFolder, bool changeimage, bool delete)
        {
            ...
            YFDirectory inFile_Folder = inFile.Parent;
            ...
            using (BinaryStream bs_memory = new BinaryStream())
            {
                using (BinaryStream bs_origin = inFile.OpenAsBinaryStream())
                {
                    ...
                    YFDirectory lst = outFolder.CreateYFDirectory("POPSTUDIOINFO");
                    int compresslevel = 0;
                    bool getlevel = true;
                    if (rsb.head.xmlPart1_BeginOffset != 0)
                    {
                        using (BinaryStream bs2 = new BinaryStream())
                        {
                            ...
                            bs2.Position = 0;
                            XmlConvert.DatToXml(bs2, lst.CreateYFFile("RESOURCES.XML"));
                        }
                    }
                    using (Stream stream = lst.CreateYFFile("COMPOSITERESOURCES.XML").OpenAsStream())
                    {
                    ...
                    using (Stream stream = lst.CreateYFFile("RESOURCESGROUP.XML").OpenAsStream())
                    {
                        using (StreamWriter sw = new StreamWriter(stream))
                        {
                            sw.WriteLine("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
                            ...
                            for (int i = 0; i < rsb.head.rsgp_Number; i++)
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        // rsgp is out of rsb
                                        YFFile guess_rsg_name = inFile_Folder.GetYFFile(rsb.rsgpInfo[i].ID);
                                        if (guess_rsg_name is null)
                                        {
                                            guess_rsg_name = inFile_Folder.GetYFFile(rsb.rsgpInfo[i].ID + ".rsg");
                                            if (guess_rsg_name is null)
                                            {
                                                guess_rsg_name = inFile_Folder.GetYFFile(rsb.rsgpInfo[i].ID + ".rsgp");
                                                if (guess_rsg_name is null)
                                                {
                                                    guess_rsg_name = inFile_Folder.GetYFFile(rsb.rsgpInfo[i].ID + ".rsg.smf");
                                                    if (guess_rsg_name is null)
                                                    {
                                                        guess_rsg_name = inFile_Folder.GetYFFile(rsb.rsgpInfo[i].ID + ".rsgp.smf");
                                                        if (guess_rsg_name is null)
                                                        {
                                                            continue;
                                                        }
                                        ...
                                        rback = 0;
                                        innerRsgpFile = guess_rsg_name.OpenAsBinaryStream();
                                    ...
                                    using (BinaryStream bs_p0 = new BinaryStream())
                                    {
                                        using (BinaryStream bs_p1 = new BinaryStream())
                                        {
                                            ...
                                            for (int xhi = 0; xhi < rsgp.fileList.Length; xhi++)
                                            {
                                                ...
                                                YFFile nname = CreateYFFileFromPath((adds + str.name).Replace('\\', '/').Replace("//", "/"), outFolder);
                                                if (str.type == 1)
                                                {
                                                    var p0ex = (RsgpPart0ExtraInfo)str.extraInfo;
                                                    if (p0ex == null) throw new Exception();
                                                    bs_p0.Position = p0ex.offset;
                                                    using (BinaryStream bs4 = nname.CreateAsBinaryStream())
                                                    {
                                                ...
                                                else if (str.type == 2)
                                                {
                                                    ...
                                                    if (changeimage)
                                                    {
                                                        ...
                                                        {
                                                            YFAPI.DecodeImage(nname, nname.Parent.CreateYFFile(nname.GetNameWithoutExtension() + ".png"), 0);
                                                            if (delete)
                                                            {
                                                                nname.DeleteSelf();
                                                            }
                    ...
                    if (usePoolXml)
                    {
                        using (Stream stream_poolinfo = lst.CreateYFFile("POOL.XML").OpenAsStream())
                        {
                            using (StreamWriter sw = new StreamWriter(stream_poolinfo))
                            {
                                sw.WriteLine("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
                    ...
                    using (Stream stream_packinfo = lst.CreateYFFile("PACKINFO.XML").OpenAsStream())
                    {
                        using (StreamWriter sw = new StreamWriter(stream_packinfo))
                        {
                            sw.WriteLine("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
