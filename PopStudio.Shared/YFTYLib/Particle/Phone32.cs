...
{
    internal class Phone32
    {
        public static void Encode(Particle particles, YFFile outFile, Func<object, int> func)
        {
            using (BinaryStream bs = new BinaryStream())
            {
                ...
                {
                    ParticleEmitter emitter = particles.Emitters[i];
                    bs.WriteInt32(func(emitter.Image));
        ...
        public static Particle Decode(YFFile inFile, Func<int, object> func)
        {
            using (BinaryStream bs = new BinaryStream())
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ParticleEmitter emitter = particles.Emitters[i];
                    int tempint = bs.ReadInt32();
                    if (tempint != -1) emitter.Image = func(tempint);
